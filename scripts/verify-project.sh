...
printf '%s\n' "Checking the store-readiness evidence contract..."
PYTHONDONTWRITEBYTECODE=1 python3 "$ROOT_DIR/scripts/ci/verify_store_readiness_evidence.py" \
  "$ROOT_DIR/docs/evidence/store-readiness-template.json"
if PYTHONDONTWRITEBYTECODE=1 python3 "$ROOT_DIR/scripts/ci/verify_store_readiness_evidence.py" \
  "$ROOT_DIR/docs/evidence/store-readiness-template.json" --require-recorded >/dev/null 2>&1; then
  echo "Committed store-readiness template must not pass as recorded evidence." >&2; exit 1
fi
...
printf '%s\n' "Checking the protected stage deployment evidence contract..."
PYTHONDONTWRITEBYTECODE=1 python3 "$ROOT_DIR/scripts/ci/verify_stage_deployment_evidence.py" \
  "$ROOT_DIR/docs/evidence/digitalocean-stage-deployment-template.json"
if PYTHONDONTWRITEBYTECODE=1 python3 "$ROOT_DIR/scripts/ci/verify_stage_deployment_evidence.py" \
  "$ROOT_DIR/docs/evidence/digitalocean-stage-deployment-template.json" \
  --require-validated >/dev/null 2>&1; then
  echo "Committed stage template must not pass as validated evidence." >&2; exit 1
fi
...
printf '%s\n' "Checking the protected incident rollback evidence contract..."
PYTHONDONTWRITEBYTECODE=1 python3 "$ROOT_DIR/scripts/ci/verify_incident_rollback_evidence.py" \
  "$ROOT_DIR/docs/evidence/incident-rollback-drill-template.json"
if PYTHONDONTWRITEBYTECODE=1 python3 "$ROOT_DIR/scripts/ci/verify_incident_rollback_evidence.py" \
  "$ROOT_DIR/docs/evidence/incident-rollback-drill-template.json" \
  --require-validated >/dev/null 2>&1; then
  echo "Committed incident template must not pass as validated evidence." >&2; exit 1
fi
PYTHONDONTWRITEBYTECODE=1 python3 "$ROOT_DIR/scripts/ci/test_verify_incident_rollback_evidence.py"
