...
def recorded(ready: bool = True) -> dict:
    ...
    data["stores"] = [{"platform": platform, "accountType": "organization", "accountStatus": "VERIFIED", "appRecordStatus": "CREATED", "applicationId": f"com.walkingrpg.{platform}", "ownerRole": "store_account_owner", "nextActionDueAtUtc": None, "blockerCategory": None} for platform in ("apple", "google")]
    ...
    data["approval"] = {"productOwnerRole": "product_owner", "releaseOwnerRole": "release_owner"}
    if not ready:
        item = data["stores"][1]; item.update({"accountStatus": "BLOCKED", "appRecordStatus": "BLOCKED", "applicationId": None, "nextActionDueAtUtc": "2026-09-10T09:00:00Z", "blockerCategory": "verification_pending"})
    return data
...
class AccountReadinessTest(unittest.TestCase):
    ...
    def test_final_ids_and_both_stores_are_required(self) -> None:
        data = recorded(); data["stores"][0]["applicationId"] = "walking-rpg"
        with self.assertRaisesRegex(V.AccountReadinessError, "reverse-domain"): V.validate(data)
