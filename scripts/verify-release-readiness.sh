...
for file in \
  ...
  backend/src/main/resources/db/migration/V31__trail_memory_secret_route.sql \
  backend/src/main/resources/db/migration/V32__energy_discipline_dawn_meridian.sql \
  backend/src/main/resources/db/migration/V33__steady_step_first_light_causeway.sql \
  backend/src/main/resources/db/migration/V34__repeatable_expedition_journeys.sql \
  ...
  backend/src/test/java/com/walkingrpg/backend/migration/AdultPetEvolutionMigrationTest.java \
  backend/src/test/java/com/walkingrpg/backend/migration/AdultPetFrontierRouteMigrationTest.java \
...
PYTHONDONTWRITEBYTECODE=1 python3 scripts/ci/test_verify_ios_pod_lock.py
...
PYTHONDONTWRITEBYTECODE=1 python3 scripts/ci/test_verify_build_tool_wrapper_pins.py
...
grep -Fq '> /usr/local/share/walking-rpg/source-git-sha' backend/Dockerfile || fail 'protected backend image must embed its source commit in a file'
grep -Fq '> /usr/local/share/walking-rpg/source-git-tree' backend/Dockerfile || fail 'protected backend image must embed its source tree in a file'
grep -Fq 'chmod 0555 /usr/local/share/walking-rpg' backend/Dockerfile || fail 'protected backend image provenance directory must not be runtime-writable'
...
grep -Fq 'readonly IMAGE_PROVENANCE_DIRECTORY=/usr/local/share/walking-rpg' backend/docker-entrypoint.sh || fail 'container provenance path must not be runtime-overridable'
...
if versions != expected or not versions or versions[-1] < 34:
    raise SystemExit(f'Flyway versions must be contiguous through at least V34: {versions}')
