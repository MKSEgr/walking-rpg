...
grep -Fq 'accountExportSnapshotTransaction.setPropagationBehavior(' backend/src/main/java/com/walkingrpg/backend/platform/application/PlatformAdminService.java || fail 'account export must create its snapshot only after the outer deletion boundary'
grep -Fq 'TransactionDefinition.PROPAGATION_REQUIRES_NEW' backend/src/main/java/com/walkingrpg/backend/platform/application/PlatformAdminService.java || fail 'account export snapshot must not release the outer subject lock'
grep -Fq '@Transactional(isolation = Isolation.READ_COMMITTED)' backend/src/main/java/com/walkingrpg/backend/platform/application/PlatformAdminService.java || fail 'account export deletion check must observe a commit after waiting for the subject lock'
