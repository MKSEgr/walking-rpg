...
> **Расширение:** ADR 0014 сохраняет идемпотентное разрешение и progression, но первое событие теперь продолжает экспедицию, а окончательное `COMPLETED` наступает после второго события.
