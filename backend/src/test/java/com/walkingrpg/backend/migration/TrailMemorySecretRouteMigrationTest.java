...
@Testcontainers
class TrailMemorySecretRouteMigrationTest {

    @Container
    static final PostgreSQLContainer POSTGRES = PostgresTestContainer.create();

    @Test
    void shouldStageV15WithoutReplacingV14OrChangingObservatoryJourney()
            throws Exception {
        Flyway.configure()
                .dataSource(
                        POSTGRES.getJdbcUrl(),
        ...
        try (Connection connection = connection();
            ...
            assertEquals(1, scalar(statement, """
                    ...
                    WHERE content_version = 'chapter-1-v15'
                      ...
                      AND content_json ->> 'nodeCount' = '28'
                      AND content_json ->> 'topology' =
                          'trail-memory-secret-route-v1'
                    """));
            assertEquals(1, scalar(statement, """
                    SELECT count(*)
                    FROM content_release
            ...
            assertEquals(1, scalar(statement, """
                    ...
                    WHERE user_id = 'trail-memory-migration-user'
                      AND state_json -> 'unlockedSkills' ? 'trail-memory'
                      AND state_json ->> 'seasonXp' = '100'
                      AND version = 10
                    """));
            assertEquals(1, scalar(statement, """
                    SELECT count(*)
            ...
            assertEquals(1, scalar(statement, """
                    ...
                    WHERE user_id = 'trail-memory-migration-user'
                      AND current_node_id = 'hidden-signal-observatory'
                      AND status = 'EVENT_READY'
                      AND unlocked_event_id = 'hidden-signal-observatory-v1'
                      AND version = 53
                    """));
            assertEquals(1, scalar(statement, """
                    SELECT count(*)
    ...
    private void seedV30State() throws Exception {
        try (Connection connection = connection();
             Statement statement = connection.createStatement()) {
            statement.executeUpdate("""
                    INSERT INTO app_user (user_id, created_at, last_seen_at)
                    VALUES ('trail-memory-migration-user', now(), now())
                    """);
            statement.executeUpdate("""
                    INSERT INTO roadmap_user_state (
                        user_id, state_json, version, created_at, updated_at
                    ) VALUES (
                        'trail-memory-migration-user',
                        '{"seasonXp":100,"unlockedSkills":["trail-memory"]}'::jsonb,
                        10, now(), now()
                    )
                    """);
            statement.executeUpdate("""
                    INSERT INTO pilot_progress (
                        ...
                        'trail-memory-migration-user', 'navigator-v1', 4, 484,
                        640, 7, now(), now()
                    )
                    """);
            statement.executeUpdate("""
                    INSERT INTO pet_progress (
                        ...
                        'trail-memory-migration-user', 'spark-v1', 3, 234,
                        8, now(), now()
                    )
                    """);
            statement.executeUpdate("""
                    INSERT INTO expedition_progress (
                        ...
                        'trail-memory-migration-user',
                        'starter-expedition-v1', 'hidden-signal-observatory',
                        90, 90, 'EVENT_READY',
                        'hidden-signal-observatory-v1', 53, now(), now()
                    )
                    """);
            statement.executeUpdate(
