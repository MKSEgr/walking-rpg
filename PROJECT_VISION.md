...
Для snapshot с явной policy Home также объясняет, что принятые на текущей
server-owned локальной дате шаги не повышают цель этой даты: они входят только
в окна будущих личных целей. Legacy snapshot без policy не получает такого
inferred обещания.
...
поэтому отдых не выполняет отдельный reset. Home явно объясняет, что для
active day достаточно любой принятой активности, а достижение личной дневной
цели является отдельной мерой. Полная server-authoritative лента
...
Текущая Home-карточка показывает принятые из backend значения текущего XP и
порога следующего уровня, а также точный остаток и ограниченный индикатор.
Mobile валидирует этот snapshot до отображения, но не рассчитывает пороги,
level-up или награды и не обещает конкретный бонус за достижение уровня.

Platform-карточки навыков используют принятый сезонный XP и server-authored
порог. Для закрытого навыка mobile показывает точный остаток до открытия, но
не меняет сам порог, доступность команды или результат разблокировки.

Незавершённые Platform-задания с шаговой или событийной метрикой используют
принятые `progress` и `target`, чтобы показать точный остаток до выполнения.
Mobile не выводит такой остаток для неизвестной метрики и не меняет ready,
claimed, награды или claim command.

Сезонный блок получает количество уровней и XP на уровень из Platform catalog.
Mobile показывает точный остаток до следующего reward level только при наличии
этого server-authored порога, не описывает содержимое награды и не меняет
claimability. Старый cached snapshot без порога сохраняет прежнее получение
наград без client-inferred guidance.
...
Текущая Platform-карточка показывает принятые из backend bond и порог следующей
эволюции, а для растущего спутника — точный остаток в RU/EN. Mobile не владеет
порогом, не меняет готовность к эволюции и не обещает награду за её достижение.
